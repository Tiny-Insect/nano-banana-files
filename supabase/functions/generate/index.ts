...
serve(async (req) => {
  ...
  try {
    ...
    const postWithBody = (body: Record<string, any>) => fetch(chatUrl, {
      method: "POST",
      headers: reqHeaders,
    ...
    // Compatibility fallback for Google native API when some fields are not supported.
    // Different model versions may reject optional fields like thinkingConfig/imageConfig.
    if (!response.ok && isGoogle && response.status === 400) {
      ...
      for (const variant of variants) {
        ...
        if (trial.ok) {
          ...
          requestBody = variant;
          errorText = "";
          console.log("Google API compatibility fallback succeeded");
          break;
        }
        errorText = await trial.text();
      }
    }
