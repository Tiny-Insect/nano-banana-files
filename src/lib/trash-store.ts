...
function createTrashItemId(baseId: string): string {
  return `${baseId}-img-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}

function createRestoreTaskId(baseId: string): string {
  return `${baseId}-restored-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}
...
/** Move a single image from a task to trash (image-level delete). */
export function moveImageToTrash(task: GenerationTask, imageIndex: number): boolean {
  const imageUrl = task.generatedImages?.[imageIndex];
  if (!imageUrl) return false;

  const thumbUrl = task.thumbnails?.[imageIndex] || imageUrl;
  const trash = loadTrash();
  const trashedTask: TrashedTask = {
    ...task,
    id: createTrashItemId(task.id),
    numImages: 1,
...
/** Restore a single image from trash as its own task. */
export function restoreImageFromTrash(taskId: string, imageIndex: number): GenerationTask | null {
  const trash = loadTrash();
  const idx = trash.findIndex((t) => t.id === taskId);
  if (idx === -1) return null;

  const item = trash[idx];
  const imageUrl = item.generatedImages?.[imageIndex];
  if (!imageUrl) return null;
  const thumbUrl = item.thumbnails?.[imageIndex] || imageUrl;

  if ((item.generatedImages?.length || 0) <= 1) {
    trash.splice(idx, 1);
  } else {
    item.generatedImages = item.generatedImages.filter((_, i) => i !== imageIndex);
    item.thumbnails = (item.thumbnails || []).filter((_, i) => i !== imageIndex);
    trash[idx] = item;
  }
  saveTrash(trash);

  const { deletedAt, ...baseTask } = item;
  return {
    ...(baseTask as GenerationTask),
    id: createRestoreTaskId(baseTask.id),
    numImages: 1,
    status: "complete",
    generatedImages: [imageUrl],
    thumbnails: [thumbUrl],
...
/** Permanently delete a single image from trash. */
export async function permanentDeleteImage(taskId: string, imageIndex: number): Promise<void> {
  const trash = loadTrash();
  const idx = trash.findIndex((t) => t.id === taskId);
  if (idx === -1) return;
  ...
  if (imageUrl) {
    try {
      ...
      if (thumbUrl && thumbUrl !== imageUrl) {
        await storage.deleteImageByUrl(thumbUrl);
      }
    } catch {
      // Best-effort
    }
