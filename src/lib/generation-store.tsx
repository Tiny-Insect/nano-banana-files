...
function isValidStoredUrl(url: string): boolean {
  return url.startsWith("http") || url.startsWith("local-file://") || url === "[upload-failed]";
}
...
function loadTasks(): GenerationTask[] {
  try {
    ...
    return tasks.map((t) => {
      // Filter out placeholder markers from display arrays
      const cleanImages = (t.generatedImages || []).filter(u => u !== "[upload-failed]");
      const cleanThumbs = (t.thumbnails || []).filter(u => u !== "[upload-failed]");
      const hadFailures = (t.generatedImages || []).some(u => u === "[upload-failed]");
      return {
        ...t,
        generatedImages: cleanImages,
        thumbnails: cleanThumbs.length > 0 ? cleanThumbs : [],
        status: cleanImages.length > 0 ? "complete" : (t.status === "complete" || t.status === "error" ? t.status : "error" as TaskStatus),
        error: hadFailures && cleanImages.length === 0
          ? "图片保存失败，请重新生成"
          : (t.status !== "complete" && t.status !== "error" && cleanImages.length === 0)
            ? "页面刷新后任务中断"
            : t.error,
      };
    });
  ...
  try {
    const toSave = tasks.slice(-MAX_STORED_TASKS).map((t) => ({
      ...
      generatedImages: (t.generatedImages || []).map((img) =>
        isValidStoredUrl(img) ? img : "[upload-failed]"
      ),
      thumbnails: (t.thumbnails || []).map((img) =>
        isValidStoredUrl(img) ? img : "[upload-failed]"
      ),
    }));
