...
import { Download, Trash2, ImageIcon, ZoomIn, ZoomOut, AlertTriangle, RotateCcw } from "lucide-react";
...
import { loadTrash, restoreImageFromTrash, permanentDeleteImage, clearAllTrash, type TrashedTask } from "@/lib/trash-store";
...
export default function RecentlyDeleted() {
  ...
  const [deleteConfirmImage, setDeleteConfirmImage] = useState<TrashImage | null>(null);
  ...
  const handleRestore = useCallback((taskId: string, imageIndex: number) => {
    const fadeKey = `${taskId}-${imageIndex}`;
    setFadingOut((prev) => new Set(prev).add(fadeKey));
    setTimeout(() => {
      const restored = restoreImageFromTrash(taskId, imageIndex);
      if (restored) {
        setTasks((prev) => {
  ...
  const handlePermanentDelete = async (taskId: string, imageIndex: number) => {
    await permanentDeleteImage(taskId, imageIndex);
    refreshTrash();
    ...
    <Layout>
          ...
          <div style={{ columnWidth: `${colWidth}px`, columnGap: "12px" }}>
            {images.map((img) => {
              ...
              const fullSrc = resolveImageSrc(img.imageUrl);
               const isFading = fadingOut.has(`${img.taskId}-${img.imageIndex}`);
              return (
                <div
                  key={img.id}
                    ...
                    <div className="flex gap-1 shrink-0">
                      ...
                      >
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="w-7 h-7 text-white/70 hover:text-white hover:bg-white/20"
                          onClick={(e) => { e.stopPropagation(); handleRestore(img.taskId, img.imageIndex); }}
                          title="还原"
                        >
      ...
      {selectedImage && lightboxImage && (
        <ImageLightbox
          ...
          onRestore={(taskId) => {
            if (!selectedImage || selectedImage.taskId !== taskId) return;
            handleRestore(taskId, selectedImage.imageIndex);
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">该图片删除后无法找回</p>
              </div>
            </div>
            <div className="mb-4 bg-muted/20 rounded-lg p-2.5">
              <img
                src={resolveImageSrc(deleteConfirmImage.thumbUrl || deleteConfirmImage.imageUrl)}
                alt=""
                className="rounded-md object-cover w-full max-h-[220px]"
                onError={(e) => {
                  const fallback = resolveImageSrc(deleteConfirmImage.imageUrl);
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button variant="destructive" size="sm" className="h-8 px-4 text-xs" onClick={() => handlePermanentDelete(deleteConfirmImage.taskId, deleteConfirmImage.imageIndex)}>
                永久删除
              </Button>
            </div>
          </div>
            ...
            {images.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-4 bg-muted/20 rounded-lg p-2 max-h-40 overflow-y-auto custom-scrollbar">
                {images.slice(0, 20).map((img) => {
                  ...
                  return (
                    <img key={img.id} src={src} alt="" className="w-12 h-12 rounded object-cover" onError={(e) => {
                      const fallback = resolveImageSrc(img.imageUrl);
                      if (e.currentTarget.src !== fallback) {
                        e.currentTarget.src = fallback;
                      }
                    }} />
