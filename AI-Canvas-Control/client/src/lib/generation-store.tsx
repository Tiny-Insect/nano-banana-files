...
function loadTasks(): GenerationTask[] {
  try {
    ...
    return tasks.map((t) => ({
      ...t,
      // If it has images, it's complete, don't show interrupted
...
function saveTasks(tasks: GenerationTask[]) {
  try {
    // Only save the last 20 tasks to avoid localStorage quota limits with large base64 images
    const toSave = tasks.slice(-20).map((t) => ({
      ...t,
      // We MUST keep generatedImages to prevent "interrupted" state on reload
      // But we clear referenceImageBase64 which is very large
    ...
    // If quota exceeded, try saving even fewer
    try {
